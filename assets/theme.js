...
class ThemeUtils {
  ...
  #initProductGallery() {
    ...
    const images = Array.from(thumbnails).map(thumb => {
      ...
      const src = img.src;
      return src.replace(/width=\d+/, 'width=800').replace(/height=\d+/, '');
    }).filter(Boolean);
    ...
    const updateMainImage = (index) => {
      if (index >= 0 && index < images.length) {
        ...
        mainImage.srcset = `${newSrc.replace('width=800', 'width=400')} 400w, ${newSrc} 800w, ${newSrc.replace('width=800', 'width=1200')} 1200w`;
  ...
  #initVariantSelection() {
    const variantContainer = document.querySelector('[data-product-variants]');
    if (!variantContainer) return;

    const form = variantContainer.closest('form');
    if (!form) return;

    const variantInput = form.querySelector('[data-variant-id]');
    const colorSwatches = variantContainer.querySelectorAll('[data-color-value]');
    const selects = variantContainer.querySelectorAll('select[data-option-position]');

    // Get all product variants from the page
    ...
    const updateVariant = () => {
      ...
      selects.forEach(select => {
        const position = select.getAttribute('data-option-position');
        selectedOptions[position] = select.value;
      });

      colorSwatches.forEach(swatch => {
        if (swatch.classList.contains('product-variants__color-swatch--active')) {
          const position = swatch.getAttribute('data-option-position');
          selectedOptions[position] = swatch.getAttribute('data-color-value');
        }
      });
      ...
      const matchingVariant = variants.find(variant => {
        return variant.options.every((option, index) => {
          ...
          return selectedOptions[optionPosition] === option;
        });
      });
      ...
      if (matchingVariant && variantInput) {
        ...
        const priceElement = document.querySelector('.main-product__price-current');
        ...
        const comparePriceElement = document.querySelector('.main-product__price-compare');
        if (matchingVariant.compare_at_price && matchingVariant.compare_at_price > matchingVariant.price) {
          const comparePrice = (matchingVariant.compare_at_price / 100).toFixed(2);
          if (comparePriceElement) {
            comparePriceElement.textContent = new Intl.NumberFormat(document.documentElement.lang || 'en', {
              style: 'currency',
              currency: 'USD'
            }).format(comparePrice).replace('USD', '₹');
        ...
        if (matchingVariant.compare_at_price && matchingVariant.compare_at_price > matchingVariant.price) {
          ...
          const badgeElement = document.querySelector('.main-product__badge');
          if (badgeElement) {
        ...
        if (addToCartBtn) addToCartBtn.setAttribute('data-variant-id', matchingVariant.id);
        if (buyNowBtn) buyNowBtn.setAttribute('data-variant-id', matchingVariant.id);
    ...
    colorSwatches.forEach(swatch => {
      swatch.addEventListener('click', () => {
        ...
        // Remove active from all swatches with same position
        colorSwatches.forEach(s => {
          if (s.getAttribute('data-option-position') === position) {
            s.classList.remove('product-variants__color-swatch--active');
          }
        });
        
        swatch.classList.add('product-variants__color-swatch--active');
        updateVariant();
      });
    });
  ...
  #initQuantitySelector() {
    const quantityContainer = document.querySelector('[data-product-quantity]');
    if (!quantityContainer) return;

    const input = quantityContainer.querySelector('[data-quantity-input]');
    const decreaseBtn = quantityContainer.querySelector('[data-quantity-decrease]');
    const increaseBtn = quantityContainer.querySelector('[data-quantity-increase]');
      ...
      button.addEventListener('click', () => {
        ...
        if (shareType === 'copy') {
          ...
          navigator.clipboard.writeText(url).then(() => {
            ...
            button.innerHTML = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M16.5 5.5L7.5 14.5L3.5 10.5" stroke="#28D98E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
            setTimeout(() => {
